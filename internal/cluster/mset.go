...
func MGet(cluster *Cluster, c slava.Connection, cmdLine aof.CmdLine) slava.Reply {
	if len(cmdLine) < 2 {
...
func MSet(cluster *Cluster, c slava.Connection, cmdLine aof.CmdLine) slava.Reply {
...
func MSetNX(cluster *Cluster, c slava.Connection, cmdLine aof.CmdLine) slava.Reply {
	argCount := len(cmdLine) - 1
	if argCount%2 != 0 || argCount < 1 {
...
func prepareMSetNx(cluster *Cluster, conn slava.Connection, cmdLine aof.CmdLine) slava.Reply {
	args := cmdLine[1:]
	if len(args)%2 != 0 {
