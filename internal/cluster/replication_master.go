...
type masterStatus struct {
	...
	replId       string
	backlog      *replBacklog
	slaveMap     map[slava.Connection]*slaveClient
...
func (server *database.Server) bgSaveForReplication() {
	go func() {
		defer func() {
			if e := recover(); e != nil {
...
func (server *database.Server) saveForReplication() error {
...
func (server *database.Server) rewriteRDB() error {
	rdbFile, err := ioutil.TempFile("", "*.rdb")
	if err != nil {
		return fmt.Errorf("create temp rdb failed: %v", err)
	}
...
func (server *database.Server) masterFullReSyncWithSlave(slave *slaveClient) error {
...
func (server *database.Server) masterTryPartialSyncWithSlave(slave *slaveClient, replId string, slaveOffset int64) error {
	server.masterStatus.mu.RLock()
...
func (server *database.Server) masterSendUpdatesToSlave() error {
...
func (server *database.Server) execPSync(c slava.Connection, args [][]byte) slava.Reply {
...
func (server *database.Server) execReplConf(c slava.Connection, args [][]byte) slava.Reply {
	if len(args)%2 != 0 {
...
func (server *database.Server) removeSlave(slave *slaveClient) {
	server.masterStatus.mu.Lock()
	defer server.masterStatus.mu.Unlock()
	_ = slave.conn.Close()
...
func (server *database.Server) setSlaveOnline(slave *slaveClient, currentOffset int64) {
	server.masterStatus.mu.Lock()
	defer server.masterStatus.mu.Unlock()
...
func (server *database.Server) masterCron() {
	server.masterStatus.mu.Lock()
...
func (listener *replAofListener) Callback(cmdLines []aof.CmdLine) {
...
func (server *database.Server) initMaster() {
	server.masterStatus = &masterStatus{
		mu:           sync.RWMutex{},
		replId:       utils.RandHexString(40),
		backlog:      &replBacklog{},
		slaveMap:     make(map[slava.Connection]*slaveClient),
		waitSlaves:   make(map[*slaveClient]struct{}),
		onlineSlaves: make(map[*slaveClient]struct{}),
...
func (server *database.Server) stopMaster() {
	server.masterStatus.mu.Lock()
	defer server.masterStatus.mu.Unlock()
	...
	server.masterStatus.slaveMap = make(map[slava.Connection]*slaveClient)
