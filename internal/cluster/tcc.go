...
func NewTransaction(cluster *Cluster, c slava.Connection, id string, cmdLine [][]byte) *Transaction {
	return &Transaction{
		id:      id,
		cmdLine: cmdLine,
		cluster: cluster,
...
func execPrepare(cluster *Cluster, c slava.Connection, cmdLine aof.CmdLine) slava.Reply {
	if len(cmdLine) < 3 {
...
func execRollback(cluster *Cluster, c slava.Connection, cmdLine aof.CmdLine) slava.Reply {
	if len(cmdLine) != 2 {
...
func execCommit(cluster *Cluster, c slava.Connection, cmdLine aof.CmdLine) slava.Reply {
	if len(cmdLine) != 2 {
...
func requestCommit(cluster *Cluster, c slava.Connection, txID int64, groupMap map[string][]string) ([]slava.Reply, protocol.ErrorReply) {
	var errReply protocol.ErrorReply
	txIDStr := strconv.FormatInt(txID, 10)
	respList := make([]slava.Reply, 0, len(groupMap))
...
// groupMap: node -> keys
func requestRollback(cluster *Cluster, c slava.Connection, txID int64, groupMap map[string][]string) {
	txIDStr := strconv.FormatInt(txID, 10)
	for node := range groupMap {
		if node == cluster.self {
...
func (cluster *Cluster) relayPrepare(node string, c slava.Connection, cmdLine aof.CmdLine) slava.Reply {
	if node == cluster.self {
