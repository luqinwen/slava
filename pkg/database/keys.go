...
func execDel(db *DB, args [][]byte) slava.Reply {
...
func execExists(db *DB, args [][]byte) slava.Reply {
	result := int64(0)
...
func execType(db *DB, args [][]byte) slava.Reply {
	key := string(args[0])
...
// execRename a key
func execRename(db *DB, args [][]byte) slava.Reply {
	if len(args) != 2 {
...
func execRenameNx(db *DB, args [][]byte) slava.Reply {
	src := string(args[0])
	dest := string(args[1])
...
func execExpire(db *DB, args [][]byte) slava.Reply {
	key := string(args[0])
...
func execExpireAt(db *DB, args [][]byte) slava.Reply {
	key := string(args[0])
...
func execPExpire(db *DB, args [][]byte) slava.Reply {
	key := string(args[0])
...
func execPExpireAt(db *DB, args [][]byte) slava.Reply {
	key := string(args[0])
...
func execTTL(db *DB, args [][]byte) slava.Reply {
	key := string(args[0])
	_, exists := db.GetEntity(key)
	if !exists {
...
func execPTTL(db *DB, args [][]byte) slava.Reply {
	key := string(args[0])
	_, exists := db.GetEntity(key)
	if !exists {
		return protocol.MakeIntReply(-2)
...
func execPersist(db *DB, args [][]byte) slava.Reply {
	key := string(args[0])
	_, exists := db.GetEntity(key)
	if !exists {
		return protocol.MakeIntReply(0)
...
func execKeys(db *DB, args [][]byte) slava.Reply {
...
func execCopy(mdb *Server, conn slava.Connection, args [][]byte) slava.Reply {
