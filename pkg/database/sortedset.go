...
func execZScore(db *DB, args [][]byte) slava.Reply {
	// parse args
	key := string(args[0])
...
func execZRank(db *DB, args [][]byte) slava.Reply {
	// parse args
	key := string(args[0])
	member := string(args[1])
...
func execZRevRank(db *DB, args [][]byte) slava.Reply {
	// parse args
	key := string(args[0])
	member := string(args[1])
...
func execZCard(db *DB, args [][]byte) slava.Reply {
	// parse args
	key := string(args[0])
...
func execZRange(db *DB, args [][]byte) slava.Reply {
	// parse args
...
func execZRevRange(db *DB, args [][]byte) slava.Reply {
	// parse args
	if len(args) != 3 && len(args) != 4 {
...
func range0(db *DB, key string, start int64, stop int64, withScores bool, desc bool) slava.Reply {
	// get data
	sortedSet, errReply := db.getAsSortedSet(key)
	if errReply != nil {
		return errReply
...
func execZCount(db *DB, args [][]byte) slava.Reply {
	key := string(args[0])
 ...
 */
func rangeByScore0(db *DB, key string, min *SortedSet.ScoreBorder, max *SortedSet.ScoreBorder, offset int64, limit int64, withScores bool, desc bool) slava.Reply {
	// get data
	sortedSet, errReply := db.getAsSortedSet(key)
	if errReply != nil {
		return errReply
...
func execZRangeByScore(db *DB, args [][]byte) slava.Reply {
	if len(args) < 3 {
...
func execZRevRangeByScore(db *DB, args [][]byte) slava.Reply {
	if len(args) < 3 {
		return protocol.MakeErrReply("ERR wrong number of arguments for 'zrangebyscore' command")
	}
	key := string(args[0])
...
func execZRemRangeByScore(db *DB, args [][]byte) slava.Reply {
	if len(args) != 3 {
...
func execZRemRangeByRank(db *DB, args [][]byte) slava.Reply {
	key := string(args[0])
	start, err := strconv.ParseInt(string(args[1]), 10, 64)
	if err != nil {
		return protocol.MakeErrReply("ERR value is not an integer or out of range")
...
func execZPopMin(db *DB, args [][]byte) slava.Reply {
	key := string(args[0])
	count := 1
	if len(args) > 1 {
		var err error
...
func execZRem(db *DB, args [][]byte) slava.Reply {
	// parse args
	key := string(args[0])
...
func execZIncrBy(db *DB, args [][]byte) slava.Reply {
	key := string(args[0])
